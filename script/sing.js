...
if (urlYtb) {
  ...
  videoID = match ? match[1] : null;
    ...
    return  api.sendMessage({
      body: title,
      attachment: await bela(downloadLink,'audio.mp3')
...
thumbnails.push(belaSt(info.thumbnail,'photo.jpg'));
...
async function bela(url,pathName) {
  try {
...
async function belaSt(url,pathName) {
  try {
    const response = await axios.get(url,{
      responseType: "stream"
    });
